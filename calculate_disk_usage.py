...
def diskUsed():
    ...
    result_used = (
        float(result_used[1]) if "." in result_used[1] else int(result_used[1])
    )
    return result_used
...
def elkSnapsSize():
    ...
    opt_bsi_data = subprocess.check_output(
        "cd /opt/bsi/data && du -sh elasticsearch", shell=True
    ).decode()
    pattern_elk = r"(\d+(?:\.\d+)?)"
...
def diskAvail():
    ...
    result_avail = (
        float(result_avail[2]) if "." in result_avail[2] else int(result_avail[2])
    )
    return result_avail
